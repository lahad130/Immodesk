...
export default async function LoyersPage() {
  ...
  return (
    <div className="space-y-5 max-w-5xl">
      <div>
        <h2 className="text-lg font-semibold text-[#181818]">Loyers</h2>
        <p className="text-sm text-[#62605B]">Vue d&apos;ensemble des encaissements</p>
      </div>

      {/* Métriques */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-white border border-[#181818]/[0.08] rounded-2xl p-5">
          <p className="text-xs text-[#A09E96] uppercase tracking-wide font-semibold mb-2">Attendus ce mois</p>
          <p className="text-2xl font-bold text-[#181818]">{formatFCFA(loyersAttendus)}</p>
          <p className="text-xs text-[#91908C] mt-1">{(activeLeases ?? []).length} bail{(activeLeases ?? []).length !== 1 ? 's' : ''} actif{(activeLeases ?? []).length !== 1 ? 's' : ''}</p>
        </div>
        <div className="bg-white border border-[#181818]/[0.08] rounded-2xl p-5">
          <p className="text-xs text-[#A09E96] uppercase tracking-wide font-semibold mb-2">Reçus ce mois</p>
          <p className="text-2xl font-bold text-[#1F8A5B]">{formatFCFA(loyersRecus)}</p>
          <p className="text-xs text-[#91908C] mt-1">Paiements validés</p>
        </div>
        <div className="bg-white border border-[#181818]/[0.08] rounded-2xl p-5">
          <p className="text-xs text-[#A09E96] uppercase tracking-wide font-semibold mb-2">Taux recouvrement</p>
          <p className={`text-2xl font-bold ${tauxRecouvrement >= 80 ? 'text-[#1F8A5B]' : tauxRecouvrement >= 50 ? 'text-orange-600' : 'text-red-600'}`}>
            {tauxRecouvrement}%
          </p>
          <p className="text-xs text-[#91908C] mt-1">{formatFCFA(loyersAttendus - loyersRecus)} restant</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#181818]/[0.08] rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-[#181818]/[0.08]">
          <h3 className="text-sm font-semibold text-[#181818]">Revenus 6 derniers mois</h3>
        </div>
        <div className="p-5">
      ...
      <div className="bg-white border border-[#181818]/[0.08] rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-[#181818]/[0.08]">
          <h3 className="text-sm font-semibold text-[#181818]">Impayés — relances WhatsApp</h3>
        ...
        <div className="divide-y divide-[#181818]/[0.06]">
          {(retardPayments ?? []).length === 0 ? (
            <div className="px-5 py-10 text-center">
              <p className="text-sm text-[#A09E96]">Aucun impayé</p>
            </div>
          ) : (
              ...
              return (
                <div key={pmt.id as string} className="flex items-center gap-4 px-5 py-3.5">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#181818] truncate">{tenant?.full_name ?? '—'}</p>
                    <p className="text-xs text-[#91908C] truncate">{propertyTitle}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-sm font-semibold text-red-600">{formatFCFA(pmt.amount_fcfa as number)}</p>
                  ...
                  {tenant?.whatsapp ? (
                    <a
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-green-600/15 hover:bg-green-600/25 text-green-700 text-xs font-medium rounded-lg transition shrink-0"
                    >
                    ...
                    <span className="text-xs text-[#B5B3AB] w-24 text-right shrink-0">Pas de WhatsApp</span>
                  )}
                </div>
              )
            })
